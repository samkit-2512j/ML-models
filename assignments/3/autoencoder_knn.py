...
def spotify_1():
    ...
    autoen = Autoencoder(input_dim=X.shape[1], encoding_dim=best_dimension, hidden_layers=[32,16,8], activation=tanh, batch_size=32)
    autoen.fit(X=X,X_val=X)

    print(autoen.get_latent(X).shape)
    print("Reconstruction error: ",autoen.reconstruction_error(X))

    X_red=autoen.get_latent(X)
    n=5
    best_metric='minkowski'

    X_train_red,y_train,X_val_red,y_val,X_test_red,y_test=split(X_red,y)
    X_train,y_train,X_val,y_val,X_test,y_test=split(X,y)
    ...
    knn_autoencoder=KNN(k=n,dist_metric=best_metric)
    knn_autoencoder.fit(X_train_red, y_train)

    y_pred_autoencoder = knn_autoencoder.predict(X_test_red)
    pf_autoencoder=PerfMeeasures(y_test,y_pred_autoencoder)
    accuracy=pf_autoencoder.accuracy()
    recall=pf_autoencoder.class_recall()
    precision=pf_autoencoder.class_precision()
    f1score=pf_autoencoder.f1_score_all(mode='macro')
    time_taken_pca=pf_autoencoder.measure_the_inference_time_for_plotting(knn_autoencoder,X_test_red)
    print(f"Autoencoder accuracy is: {accuracy:.4f}")
    print(f"Autoencoder recall is: {np.mean(recall)}")
    print(f"Autoencoder precision is: {np.mean(precision)}")
    print(f"Autoencoder F1-score is: {f1score:.4f}")
